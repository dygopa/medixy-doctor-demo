...
export default function Formulary({userId}:{
    ...
    const CenterComponent = ({data}:{
        ...
        return(
            <div onClick={()=>{ setFormData({...thisValue}) }} className="cursor-pointer w-full border rounded bg-white p-4 flex justify-between items-center gap-4">
                <span className="w-[2.5rem] h-[2.5rem] rounded-md bg-primary/30 text-primary flex flex-col justify-center items-center text-lg">
                    <BiBuildingHouse/>
                </span>
                ...
                <span className={twMerge([
                    ...
                    (formData?.parent_location_id === data.id) && "bg-green-500 border-green-500 text-white"
                ])}>
                    <FiCheck/>
                </span>
            </div>
                ...
                <div className="border w-full rounded-md p-5 flex">
                    <div className="w-full flex flex-wrap justify-between items-center gap-6 relative">
                        <div className="w-full border-b mb-2">
                            <p className="font-medium text-base text-slate-900 pb-2">Selecciona el centro médico</p>
                        </div>
                        <div className="flex justify-start items-start relative w-full gap-3">
                            <p className="w-1/5 text-[13px] text-slate-900 font-medium mb-2">Centro médico</p>
                            <div className="w-4/5 flex flex-col justify-start items-start relative gap-3">
                                ...
                                {(medicalCentersSuccess && [...medicalCenters as Array<ILocality>].length > 0) &&
                                    [...medicalCenters as Array<ILocality>].map((center, i)=> <CenterComponent key={i} data={center} />)
                                }
                                {(medicalCentersSuccess && [...medicalCenters as Array<ILocality>].length === 0) &&
                                    <div className="w-full flex flex-col justify-center items-center">
    ...
    return (
        <>
            ...
            <AlertComponent variant="error" show={createUserLocalityError !== null} description="Ha ocurrido un error inesperado en la creación" />
            <AlertComponent variant="success" show={createUserLocalitySuccess} description="Tu consultorio se ha creado exitosamente" />
