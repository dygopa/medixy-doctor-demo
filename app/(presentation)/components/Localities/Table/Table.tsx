...
export default function Table({user}:{
  ...
  const LocalityComponent = ({data}:{
    data: ILocality;
  }) => {
    return(
      <Link href={`/localities/${data.id}`} className="bg-white border rounded-lg p-4 flex flex-col justify-between items-start gap-4 cursor-pointer">
        <div className="w-full flex justify-start items-center gap-4">
          ...
          <div className="relative flex flex-col justify-center items-start">
            ...
            <p className="font-light text-sm text-slate-500">Número: {data.code}</p>
          </div>
        </div>
        <div className="w-full grid grid-cols-3 gap-1">
          <div className="flex flex-col justify-start items-start gap-2 text-left">
            ...
            <p className="font-normal text-gray-950 text-base">{data.state.name}</p>
          </div>
          <div className="flex flex-col justify-start items-start gap-2 text-left">
          ...
          <div className="flex flex-col justify-start items-start gap-2 text-left">
            ...
            <p className="font-normal text-gray-950 text-base">{data.postal_code}</p>
          </div>
  ...
  if (loading){
    return(
      <div className="w-full flex flex-col justify-center items-center">
        ...
        <p className="font-light text-slate-500 text-base">Cargando tus consultorios.</p>
      </div>
    )
  }
  ...
  if(successful && [...data as Array<ILocality>].length > 0){
    return(
      <div className="grid grid-cols-3 gap-2 overflow-auto intro-y lg:overflow-visible z-0">
