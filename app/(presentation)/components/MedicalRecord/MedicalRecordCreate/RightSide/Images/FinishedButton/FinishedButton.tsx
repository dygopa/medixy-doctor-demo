...
interface IFinishedButtonProps {
  user: IUser;
}

export default function FinishedButton({ user }: IFinishedButtonProps) {
  ...
  const onValidateForm = (valuesStorage: any) => {
    ...
    if (currentConsultationErrors.currentConsultationCount > 0) {
      queryErrors = queryErrors + `view=current-consultation&`;
    ...
    if (diagnosisErrors.diagnosisCount > 0) {
      queryErrors = queryErrors + `view=diagnosis&`;
    ...
    if (vitalSignsErrors.vitalSignsCount > 0) {
      queryErrors = queryErrors + `view=exploration&`;
  ...
  return (
    <>
      ...
      <Button
        variant="primary"
        className="h-[46px] lg:w-[300px] md:w-[300px] w-full"
