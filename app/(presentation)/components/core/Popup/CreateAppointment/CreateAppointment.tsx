...
function CreateAppointment({
  ...
  const {
    ...
    loading: loadingCreationAppointment,
    successful: loadedCreationAppointment,
    error: errorCreationAppointment,
  } = state.createAppointment;
  const {
    successful: predifinedReservationSuccessful,
    data: predifinedReservation,
  } = state.predifinedReservationData;
  const {
    successful: typeOfAppointmentCreationSuccessful,
    data: typeOfAppointmentCreation,
  } = state.typeOfAppointmentCreation;
  ...
  function getDataFromPredifined() {
    ...
    if (predifinedReservation["localityId"] !== undefined) {
      let findedLocality = listOfLocalities.find(
        (elem: ILocality) => elem.id === predifinedReservation["localityId"]
      );
    ...
    setFormData({
      ...formData,
      windowId: predifinedReservation["attentionWindowId"],
    });
  }
  ...
  useMemo(() => {
    if (loadedLists) {
      if (predifinedReservation["localityId"] !== undefined) {
        getServicesByLocality(
          user.userId,
          predifinedReservation["localityId"]
        )(dispatch);
        ...
        {!fromCalendar && (
          <div className="w-full flex flex-col justify-center items-start">
            ...
            <div className="w-full flex justify-end items-center">
              <Link
                href="/patients/new-patient"
                className="font-light text-sm text-primary underline"
              >
            ...
            {fromCalendar ? (
              <ExampleComponent
                data={{
                  ...
                  horaInicio: moment(predifinedReservation["date"])
                    .utc()
                    .format("hh:mm a"),
                  horaFin: moment(predifinedReservation["dateEnd"])
                    .utc()
                    .format("hh:mm a"),
      ...
      <div className="w-full flex flex-col justify-center items-center gap-4 sticky bottom-0 py-3 bg-white">
        <Button
          disabled={
            ...
            (!isNow && (selectedDate === "" || formData.windowId === 0)) ||
          ...
          className="w-full"
        >
          {loadingCreationAppointment ? "Agendando..." : "Guardar"}
        </Button>
        <p
          onClick={() => {
            cancelFuntion();
