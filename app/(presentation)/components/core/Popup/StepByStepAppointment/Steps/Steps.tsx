...
const Steps = ({ user, from = "SCHEDULE" }: IStepsProps) => {
  ...
  const ItemStep = ({ number, label }: { number: number; label: string }) => {
    ...
    return (
      <div
        ...
        onClick={() => {
          step > number && setStep(number)(dispatch);
        }}
        className={twMerge([
      ...
      >
        <div
          className={`shadow-xl w-[45px] h-[45px] rounded-full border 
          ${
            step === number
              ? "shadow-primary/70 border-primary bg-primary"
              : "shadow-primary/0 "
          }
          ${
        ...
        {isHover && (
          <span className="w-max h-fit p-3 bg-slate-900 text-white font-medium text-xs rounded absolute -bottom-[45px] z-10">
            {label}
          </span>
        )}
      </div>
  ...
  return (
    <div className="w-full h-fit flex flex-col justify-start items-center gap-6">
      <div
        className={
          "w-full h-fit flex flex-col justify-start items-center gap-4 sticky -top-6 py-3 bg-white z-10"
        }
      >
