...
export interface IAutocompleteInputMunActions {
    getMunicipalities: (obj: { searchQuery: string; federalEntityId?: number | null }) => (dispatch: Dispatch<any>) => {};
}

const getMunicipalities = (obj: { searchQuery: string; federalEntityId?: number | null }) => async (dispatch: Dispatch<any>) => {
    try {
        dispatch({ type: "GET_MUNICIPALITIES_LOADING" });

        const res: IGetMunicipalitiesResponse = await new MunicipalitiesUseCase().getMunicipalities({ searchQuery: obj.searchQuery, limit: 100, federalEntityId: obj.federalEntityId });
