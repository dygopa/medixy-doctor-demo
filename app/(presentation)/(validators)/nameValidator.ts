...
export class NameValidator extends Validator {
    validate_not_empty(): IValidator {
        if (this.value.trimEnd().length === 0) {
            return { isValid: false, error: { code: "field-is-required", message: "Debe escribir su nombre" } } as IValidator;
        }
    ...
    validate_max_length(max_length = 60): IValidator {
        if (this.value.trimEnd().length > max_length) {
            return { isValid: false, error: { code: "field-min-length", message: `El nombre debe contener máximo ${max_length} carácteres` } } as IValidator;
        }
    ...
    validate_regexp(): IValidator {
        if (!/^[ a-zA-ZàèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃÑÕäëïöüÿÄËÏÖÜŸçÇßØøÅåÆæœ''.\-]+$/.test(this.value.trimEnd())) {
            return { isValid: false, error: { code: "field-invalid-format", message: "El nombre debe contener solo letras, puntos o comillas simples" } } as IValidator;
        }
